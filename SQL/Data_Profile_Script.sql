...
FOR
    ...
    WHERE   c.TABLE_SCHEMA IN ( 'dbo' )  --------------------------------
			...
			AND c.TABLE_NAME = 'Boundaries' -- substitue the table name here 
    ...
    BEGIN
        ...
        IF @sql <> ''
            BEGIN
				...
				--SET @len = @q + cast(@char as varchar(10)) + @q
...
SELECT  [Schema] ,
        ...
        [Seq] ,
        [Data Type] ,
		c.[CHARACTER_MAXIMUM_LENGTH] AS [Configured_Length],
        [Avg Len/Val] ,
        [Min Len/Val] ,
        [Max Len/Val] ,
        [Min Date] ,
        ...
        [Num NULL]
--FROM    @Results
FROM    @Results r join INFORMATION_SCHEMA.COLUMNS c on r.[Column Name] = c.COLUMN_NAME and r.[Table Name] = c.TABLE_NAME
ORDER BY [Table Name] ,
        [Seq] ,
        [Column Name]
