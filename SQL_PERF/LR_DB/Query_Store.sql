...
DECLARE @LocalTime_Start DATETIME = '2021-09-10 14:00:00.000', @LocalTime_End DATETIME ='2021-09-17 19:50:00.000' ; 
...
Where 
			...
			--and [qsq].[object_id] <> OBJECT_ID(N'OperatingLimits_selectchanges')
			--and [qsq].[object_id] = OBJECT_ID(N'OperatingLimits_selectchanges')
			--and [qsq].[object_id] = OBJECT_ID(N'ChangeNotes_selectchanges')
			--and OBJECT_NAME(qsq.object_id) is not null 
			--and rs.runtime_stats_interval_id = 2 
			--and [rs].[avg_duration]/1000000.00 >= 2.0
				--and [qst].query_sql_text like '%insert into #ShiftSummariesGetSSFilter%'
