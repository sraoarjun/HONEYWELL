...
DECLARE @LocalTime_Start DATETIME = '2021-09-23 12:04:00.000', @LocalTime_End DATETIME ='2021-09-23 16:30:00.000' ; 
...
Where 
			...
			--and [qsq].[object_id] <> OBJECT_ID(N'USP_GetDeviationsDetailsForImpactCalculation')
			and [qsq].[object_id] = OBJECT_ID(N'USP_GetDeviationsDetailsForImpactCalculation')
			--and OBJECT_NAME(qsq.object_id) is not null 
			--and rs.runtime_stats_interval_id = 13 
			--and [rs].[avg_duration]/1000000.00 >= 1.0
			--and [rs].[avg_logical_io_reads] > 10000
				--and [qst].query_sql_text like '%USP_GetDeviationsDetailsForImpactCalculation%'
	 AND [rs].[execution_type] = 0
	 --AND [qsp].[plan_id] not in (7249,8352,9326)
	 --and rs.runtime_stats_interval_id = 39
--ORDER BY rs.avg_logical_io_reads desc
and rs.plan_id = 6962
ORDER BY rs.runtime_stats_interval_id asc 
