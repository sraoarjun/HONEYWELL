...
select * from 
(select object_name(object_id) as Table_name , i.name as Index_Name ,i.type_desc
from sys.indexes i 
where  i.index_id NOT IN (select s.index_id 
                          from sys.dm_db_index_usage_stats s 
                          where s.object_id=i.object_id and 
...
where  type_desc not in ('HEAP')

AND (Table_name not like '%history%' and Table_name not like '%filestream%' and table_name not like '%fulltext%'

		and Table_name not like '%ifts%' and Table_name not like 'sys%' and Table_name not like 'queue%' and Table_name
		not like 'SqlAgent%' and Table_name not like 'filetable%' )

 --or  Table_name not like '%fulltext_%' or Table_name not like '%filestream%')
