/****** Object:  StoredProcedure [dbo].[sp_GetStandingOrderDetails]    Script Date: 10/13/2016 23:38:23 PM ******/
...
BEGIN
	...
	IF EXISTS(SELECT StandingOrder_PK_ID FROM StandingOrders WHERE StandingOrder_PK_ID=@StandingOrder_PK_ID)
	BEGIN
	IF EXISTS (
			SELECT st.NAME
			FROM StandingOrders I
			INNER JOIN States St ON I.State_StateId = St.StateId
			WHERE CAST(StandingOrder_PK_ID AS NVARCHAR(100)) = CAST(@StandingOrder_PK_ID AS NVARCHAR(100))
				AND (
					st.NAME = 'DRAFT'
	...
	BEGIN
		...
		FROM StandingOrders
		WHERE CAST(StandingOrder_PK_ID AS VARCHAR(100)) = CAST(@StandingOrder_PK_ID AS NVARCHAR(100))
		IF (
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					WHERE LTRIM(UPPER(LU.NAME)) = 'STANDINGORDERLOCKINGTIMEOUT'
						AND LTRIM(LUT.NAME) = 'Standing Order Time Parameters';
					...
					BEGIN
						...
						SET LockedBy = @dolockBy
							,LockedTime = GETUTCDATE()
						WHERE CAST(StandingOrder_PK_ID AS VARCHAR(100)) = CAST(@StandingOrder_PK_ID AS VARCHAR(100))
					END
				END
			END
		END
	...
	SELECT DISTINCT I.StandingOrder_PK_ID
		...
		, (select count(*) from StandingOrderComments IComm,StandingOrderCommentTypes ICType where IComm.StandingOrder_StandingOrder_PK_ID = I.StandingOrder_PK_ID and ICType.Name='Acknowledge' and ICType.StandingOrderCommentType_PK_ID = IComm.StandingOrderComme
ntType_StandingOrderCommentType_PK_ID and (I.ActualStartTime is not null and IComm.CreatedTime >I.ActualStartTime)) AS ActiveAckCount
		, (select count(*) from StandingOrderComments IComm,StandingOrderCommentTypes ICType where IComm.StandingOrder_StandingOrder_PK_ID = I.StandingOrder_PK_ID and ICType.Name='Acknowledge' and ICType.StandingOrderCommentType_PK_ID = IComm.StandingOrderComme
ntType_StandingOrderCommentType_PK_ID and (I.ActualStartTime is not null and IComm.CreatedTime >I.ActualStartTime and IComm.CreatedBy=@queriedBy)) AS UserActiveAck
	...
	WHERE CAST(C.StandingOrder_StandingOrder_PK_ID AS NVARCHAR(100)) = CAST(@StandingOrder_PK_ID AS NVARCHAR(100))
	SELECT [FileName]
		,DisplayName
	FROM standingOrderAttachments AT
	JOIN StandingOrders I ON AT.StandingOrder_StandingOrder_PK_ID = I.StandingOrder_PK_ID
	WHERE CAST(AT.StandingOrder_StandingOrder_PK_ID AS NVARCHAR(100)) = CAST(@StandingOrder_PK_ID AS NVARCHAR(100))
	SELECT Link
		,DisplayName
	FROM StandingOrderLinks IL
	JOIN StandingOrders I ON IL.StandingOrder_StandingOrder_PK_ID = I.StandingOrder_PK_ID
	WHERE CAST(IL.StandingOrder_StandingOrder_PK_ID AS NVARCHAR(100)) = CAST(@StandingOrder_PK_ID AS NVARCHAR(100))
	SELECT 
		P.UserPrincipal,
		P.DisplayName,
		P.EmailID,
	...
	AND CAST(IA.StandingOrder_StandingOrder_PK_ID AS NVARCHAR(100)) = CAST(@StandingOrder_PK_ID AS NVARCHAR(100))
