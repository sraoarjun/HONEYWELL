ALTER PROCEDURE [dbo].[sp_GetAllStandingOrdersLite_Test]
...
BEGIN
	...
	IF( @assetCount > 0)
		BEGIN
			...
			--SEt @insertIntoTempSOLiteFinalListQry = @insertIntoTempSOLiteFinalListQry + '
			--	),count_cte as (select count(*) as TotalRowCount from Main_Cte)'
			--  Logic isOverDue Filter end
			--print @insertIntoTempSOLiteFinalListQry
			...
			insert into dbo.tempLiteLog(data_val)
			select @insertIntoTempSOLiteFinalListQry
