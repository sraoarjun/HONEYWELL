...
BEGIN
	...
	IF( @assetCount > 0)
		BEGIN
			...
			IF(@statusCount <> 0)
				INSERT INTO #TemptblStatusLite SELECT A.StatusName FROM @tblStandingOrderStatus A INNER JOIN States B ON B.ProcessType_ProcessTypeId = @ProcessID AND A.StatusName = B.Name
			ELSE
				INSERT INTO #TemptblStatusLite SELECT Name FROM States WHERE ProcessType_ProcessTypeId = @ProcessID
			...
			SELECT @LookAheadTime = CAST(LU.[Value] AS INT) From Lookups LU
			...
			SELECT @LookBackTime = CAST(LU.[Value] AS INT) From Lookups LU
			INNER JOIN LookupTypes LUT ON LUT.LookupType_PK_ID = LU.LookupType_PK_ID
			...
			SET @StateID= (SELECT StateID FROM States st WHERE st.Name = 'DRAFT' AND st.ProcessType_ProcessTypeId = @ProcessID);
			SET @MultiStateWhere = ' 
			...
			SET @StateID= (SELECT StateID FROM States st WHERE st.Name = 'PLANNED' AND st.ProcessType_ProcessTypeId = @ProcessID);
			SET @MultiStateWhere = ' 
			((CAST( I.State_StateID AS varchar(100))  =  CAST('''+ @StateID +''' AS varchar(100)) 
			) AND  ((CAST(I.PlanStartTime AS DateTime) <= CAST('''+ @CalculatedPlanEndTime +''' AS DateTime)) 
			AND  (ISNULL(I.PlanEndTime,1) = 1 OR CAST(I.PlanEndTime AS DateTime) >= CAST('''+ @CalculatedPlanStartTime +''' AS DateTime)))
			...
			SET @StateID= (SELECT StateID FROM States st WHERE st.Name = 'APPROVED' AND st.ProcessType_ProcessTypeId = @ProcessID);
			SET @MultiStateWhere = ' 
			((CAST( I.State_StateID AS varchar(100))  =  CAST('''+ @StateID +''' AS varchar(100)) 
			) AND  ((CAST(I.PlanStartTime AS DateTime) <= CAST('''+ @CalculatedPlanEndTime +''' AS DateTime)) 
			AND  (ISNULL(I.PlanEndTime,1) = 1 OR CAST(I.PlanEndTime AS DateTime) >= CAST('''+ @CalculatedPlanStartTime +''' AS DateTime)))
			...
			SET @StateID= (SELECT StateID FROM States st WHERE st.Name = 'REJECTED' AND st.ProcessType_ProcessTypeId = @ProcessID);
			SET @MultiStateWhere = ' 
			((CAST( I.State_StateID AS varchar(100))  =  CAST('''+ @StateID +''' AS varchar(100)) 
			) AND  ((CAST(I.PlanStartTime AS DateTime) <= CAST('''+ @CalculatedPlanEndTime +''' AS DateTime)) 
			AND  (ISNULL(I.PlanEndTime,1) = 1 OR CAST(I.PlanEndTime AS DateTime) >= CAST('''+ @CalculatedPlanStartTime +''' AS DateTime)))
			...
			SET @StateID= (SELECT StateID FROM States st WHERE st.Name = 'ACTIVATED' AND st.ProcessType_ProcessTypeId = @ProcessID);
			INSERT INTO @StatusDateApply VALUES('ACTIVATED', @StateID ,  '
			(CAST( I.State_StateID AS varchar(100))  =  CAST('''+ @StateID +''' AS varchar(100)) 
			AND (CAST(I.ActualStartTime AS DateTime)<= CAST('''+ @EndTime +''' AS DateTime))) ');
 
 
			SET @StateID= (SELECT StateID FROM States st WHERE st.Name = 'COMPLETED' AND st.ProcessType_ProcessTypeId = @ProcessID);
			...
			set @StandingOrderHistory='SELECT [StandingOrder_PK_ID],
												...
												Else I.PlanStartTime
												END as TimeOfActivity
										FROM [dbo].[StandingOrdersHistory] I
										INNER JOIN States St1
										ON (I.State_StateId = St1.StateId AND St1.ProcessType_ProcessTypeId = ''' + CAST(@ProcessID AS varchar(100)) + ''')      
			...
			SET @insertIntoTempSOLiteFinalListQry =     '	
						SELECT  I.StandingOrder_PK_ID,
								...
								I.[IsHistory]
											...
											INNER JOIN Assets A
												ON I.Asset_Asset_PK_ID = A.Asset_PK_ID                                                                
											INNER JOIN StandingOrderCategories Icat
											...
											LEFT OUTER JOIN StandingOrderProducts pd  
																							 ...
																							 OR	
																								(I.[IsHistory] = 0
																								 And EXISTS (SELECT TOP 1* 
																												...
																												INNER JOIN dbo.GroupDetails gd 
																													ON ug.GroupName=gd.Name AND ug.ScopeName=gd.ScopeName
																												INNER JOIN dbo.StandingOrderAssignees iasg 
																							 ...
																							 OR	
																								(I.[IsHistory] = 1
																								 And EXISTS (SELECT TOP 1* 
																												...
																												INNER JOIN dbo.GroupDetails gd 
																													ON ug.GroupName=gd.Name AND ug.ScopeName=gd.ScopeName
																												INNER JOIN dbo.StandingOrderAssigneesHistory iasg 
																													ON gd.[GroupDetail_PK_Id]=iasg.[GroupDetail_GroupDetail_PK_Id]
																														AND iasg.[StandingOrder_StandingOrder_PK_ID]=I.StandingOrder_PK_ID 
																														AND st.Name <> ''DRAFT''
																										) 
			...
			SET	@insertIntoTempSOLiteFinalListQry = ' INSERT  INTO #TempSOLiteList
							SELECT 
								[StandingOrder_PK_ID],
								[Name],
								[PlanStartTime] ,
								...
								[AssignToChildAssets],
								[IsHistory]
							FROM (' + @insertIntoTempSOLiteFinalListQry + ')t 
							WHERE '+
			...
			select  
					...
					CASE 
						WHEN [IsHistory] = 0  THEN
							 (select count(IComm.StandingOrder_StandingOrder_PK_ID) from StandingOrderComments IComm where IComm.StandingOrder_StandingOrder_PK_ID = [StandingOrder_PK_ID] and  @AckCommentTypeID = IComm.StandingOrderCommentType_StandingOrderCommentType_PK_ID and (ActualStartTime is not null and IComm.CreatedTime >ActualStartTime))
						ELSE 
							(select count(IComm.StandingOrder_StandingOrder_PK_ID) from StandingOrderCommentsHistory IComm where IComm.StandingOrder_StandingOrder_PK_ID = [StandingOrder_PK_ID] and  @AckCommentTypeID = IComm.StandingOrderCommentType_StandingOrderCommentTypeId 
and (ActualStartTime is not null and IComm.CreatedTime >ActualStartTime))  
					END  AS  [ActiveAckCount],
					CASE 
						WHEN [IsHistory] = 0  THEN
							 (select count(SOAttach.StandingOrder_StandingOrder_PK_ID) from StandingOrderAttachments SOAttach where SOAttach.StandingOrder_StandingOrder_PK_ID = [StandingOrder_PK_ID])
						ELSE 
							 (select count(SOAttach.StandingOrder_StandingOrder_PK_ID) from StandingOrderAttachmentsHistory SOAttach where SOAttach.StandingOrder_StandingOrder_PK_ID = [StandingOrder_PK_ID])
						END  AS  [AttachmentsCount],
					CASE 
						WHEN [IsHistory] = 0  THEN
							 (select count(SOLinks.StandingOrder_StandingOrder_PK_ID) from StandingOrderLinks SOLinks where SOLinks.StandingOrder_StandingOrder_PK_ID = [StandingOrder_PK_ID])
						ELSE 
							 (select count(SOLinks.StandingOrder_StandingOrder_PK_ID) from StandingOrderLinksHistory SOLinks where SOLinks.StandingOrder_StandingOrder_PK_ID = [StandingOrder_PK_ID])
						END  AS  [LinksCount]
					from #TempSOLiteList
			...
			Begin
				...
				Inner Join StandingOrderAssignees SOAssignee
			...
			Begin
				...
				SELECT 
					...
					P.UserPrincipal
				FROM #TempSOLiteList SOList
				Inner Join StandingOrderAssigneesHistory SOAssignee
			...
			Begin
			   ...
			   FROM #TempSOLiteList SOList
			   Join [dbo].[StandingOrderComments] IC
					ON (SOList.IsHistory = 0 
			   ...
			   LEFT JOIN  [dbo].[StandingOrdersActionHistory] IAH 
					ON IC.StandingOrderComment_PK_ID = IAH.[StandingOrderComment_StandingOrderComment_PK_ID]
			   LEFT JOIN  [dbo].[ActionTypes] AT ON AT.[ActionTypeId] = IAH.[ActionType_ActionTypeID]   
